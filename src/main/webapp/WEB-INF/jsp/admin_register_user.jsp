...
<script type="text/javascript">
    function check() {
        ...
        passwordError = false; // Corrected logic initialization
        passwordValid();
        return !passwordError; // Returns true if no error
    }

    function passwordValid() {
        const passwordsms = document.getElementById("msg");
        const patt = /^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*\W).{8,16}$/; // Corrected regex
        if (password.match(patt)) {
            passwordsms.innerHTML = "";
            passwordError = false;
        } else {
            passwordsms.innerHTML = "Invalid password: Must be 8-16 characters, include upper and lower case letters, a digit, and a special character.";
            passwordError = true; // Set to true on error
        }
    }
</script>
...
<div class="container container-newuser">
